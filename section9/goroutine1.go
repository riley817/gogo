...
func main() {
	...
	// 생성 방법 매우 간단. 리소스 매우 적게 사용 - Java나 python은 1MB 단위. Go는 Kb 단위
	// 즉, 수 많은 고루틴 동시 생성 실행 가능
	// 비동기적 함수 루틴 실행(매우 적은 용량 차지) - 채널을 통한 통신가능
	// 공유 메모리 사용 시에 정확한 동기화 코딩 필요
	// 싱글루틴에 비해 항상 빠른 처리 결과는 아니다.

	// 멀티 쓰레드 장점과 단점
	// 장점 : 응답성이 향상, 자원공유를 효율적으로 활용 사용. 작업이 분리되어 코드 간결
	// 단점 : 구현하기 어려움, 테스트 및 디버깅 어려움, 전체 프로세스의 사이드 이펙트, 성능 저하, 동기화 코딩 반드시 숙지, 데드락(교착상태)
	// 파이썬 스레드 추천!
